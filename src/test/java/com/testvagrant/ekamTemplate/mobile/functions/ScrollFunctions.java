...
public class ScrollFunctions {
  ...
  public void scrollDownToContainsText(String text) {
    scrollDownTo(
        By.xpath("//*[contains(@text,\'" + text + "\') or contains(@name,\'" + text + "\')]"));
  }
  ...
  public void scrollDownTo(By byOfElementToBeFound) {
    ...
    int i = 0;
    while (i < 12) {
      if (driver.findElements(byOfElementToBeFound).size() > 0) return;
  ...
  public void scrollDown(int times) {
    IntStream.range(0, times).forEach(time -> scrollDown());
  }
  ...
  public void scrollUpTo(By by) {
    ...
    int i = 0;
    while (i < 5) {
      if (driver.findElements(by).size() > 0) return;
